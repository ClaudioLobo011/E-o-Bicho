(function () {
  ...
  async function openAnalytics() {
    ...
    try {
      ...
      el("analytics-content").innerHTML =
        `<div class="grid grid-cols-2 gap-2 sm:grid-cols-3"><article class="rounded-xl bg-indigo-50 p-3"><p class="text-[10px] font-bold uppercase text-indigo-700">Itens</p><p class="text-xl font-bold">${numberValue(data.totals?.itemCount)}</p></article><article class="rounded-xl bg-gray-50 p-3"><p class="text-[10px] font-bold uppercase text-gray-500">Valor base</p><p class="text-lg font-bold">${formatMoney(data.totals?.sourceValue)}</p></article><article class="col-span-2 rounded-xl bg-emerald-50 p-3 sm:col-span-1"><p class="text-[10px] font-bold uppercase text-emerald-700">Comissão</p><p class="text-lg font-bold">${formatMoney(data.totals?.commission)}</p></article></div><div class="grid gap-3 lg:grid-cols-3">${analyticsList("Por profissional", data.byProfessional, "name")}${analyticsList("Por serviço/produto", data.byService, "name")}${analyticsList(
          "Por dia",
          (data.byDay || []).map((row) => ({
            ...row,
