(function () {
  ...
  const selectors = {
    ...
    importCollectorButton: document.getElementById('transfer-import-collector-button'),
    ...
    collectorModal: document.getElementById('transfer-collector-modal'),
    collectorFileInput: document.getElementById('transfer-collector-file'),
    collectorTableBody: document.getElementById('transfer-collector-table-body'),
    collectorCount: document.getElementById('transfer-collector-count'),
    collectorCancelButton: document.getElementById('transfer-collector-cancel'),
    collectorImportButton: document.getElementById('transfer-collector-import'),
  };
  ...
  function parseCollectorQuantity(value) {
    const normalized = normalizeFiniteNumber(value);
    if (normalized === null || normalized <= 0) return null;
    return normalized;
  }

  async function findProductByCodeOrBarcode(term) {
    ...
    const response = await fetch(`${API_CONFIG.BASE_URL}/transfers/search-products?term=${encodeURIComponent(normalizedTerm)}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      throw new Error(payload?.message || 'Nao foi possivel buscar produtos para importacao.');
    }
    ...
    const exact = products.find((product) => {
      const productCode = normalizeIdentifier(product?.cod);
      const productBarcode = normalizeIdentifier(product?.codbarras);
      const productBarcodeDigits = normalizeDigits(productBarcode);
      const termDigits = normalizeDigits(normalizedTerm);
      return (
        productCode.toLowerCase() === normalizedTerm.toLowerCase() ||
        productBarcode.toLowerCase() === normalizedTerm.toLowerCase() ||
        (termDigits && productBarcodeDigits && productBarcodeDigits === termDigits)
      );
    });
  ...
  function setCollectorModalOpen(open) {
    const modal = selectors.collectorModal;
    if (!modal) return;
    if (open) {
  ...
  function resetCollectorPreview() {
    state.collectorPreviewItems = [];
    if (selectors.collectorFileInput) {
      selectors.collectorFileInput.value = '';
    }
    renderCollectorPreviewTable();
  }

  function updateCollectorCounter() {
    const counter = selectors.collectorCount;
    if (!counter) return;
    const items = Array.isArray(state.collectorPreviewItems) ? state.collectorPreviewItems : [];
    const total = items.length;
    const selected = items.filter((item) => item && !item.error && item.selected).length;
    counter.textContent = `${total} item(ns) | ${selected} selecionado(s)`;
  }

  function renderCollectorPreviewTable() {
    const tbody = selectors.collectorTableBody;
    if (!tbody) return;

    if (!Array.isArray(state.collectorPreviewItems) || state.collectorPreviewItems.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6" class="px-4 py-8 text-center text-[11px] text-gray-500">Selecione um arquivo para carregar os itens.</td></tr>';
      updateCollectorCounter();
      return;
    }

    const rows = state.collectorPreviewItems.map((item, index) => {
      const codeText = escapeHtml(item.sku || item.barcode || item.inputCode || '-');
      const nameText = escapeHtml(item.description || item.errorMessage || 'Produto nao encontrado');
      const unitCostText = item.error ? '-' : formatCurrency(item.unitCost);
      const unitSaleText = item.error ? '-' : formatCurrency(item.unitSale);
      const rowClass = item.error ? 'bg-red-50/40' : 'bg-white';
      const checkboxDisabled = item.error ? 'disabled' : '';
      const checkboxChecked = item.selected && !item.error ? 'checked' : '';
      const quantityValue = Number.isFinite(Number(item.quantity)) ? Number(item.quantity) : 0;
      const quantityInputClass = item.error
        ? 'w-24 rounded-md border border-gray-300 px-2 py-1 text-right text-sm bg-gray-100 text-gray-400'
        : 'w-24 rounded-md border border-gray-300 px-2 py-1 text-right text-sm focus:border-primary focus:ring-2 focus:ring-primary/20';
      const quantityDisabled = item.error ? 'disabled' : '';

      return `
        <tr class="${rowClass}">
          <td class="px-3 py-2 text-center">
            <input type="checkbox" data-collector-field="selected" data-index="${index}" ${checkboxChecked} ${checkboxDisabled}>
          </td>
          <td class="px-3 py-2 font-medium text-gray-800">${codeText}</td>
          <td class="px-3 py-2 text-gray-700">${nameText}</td>
          <td class="px-3 py-2 text-right text-gray-700">${unitCostText}</td>
          <td class="px-3 py-2 text-right text-gray-700">${unitSaleText}</td>
          <td class="px-3 py-2 text-right">
            <input type="number" min="0.01" step="0.01" data-collector-field="quantity" data-index="${index}" class="${quantityInputClass}" value="${quantityValue}" ${quantityDisabled}>
          </td>
        </tr>
      `;
    });
  ...
  function attachCollectorTableEvents() {
    const tbody = selectors.collectorTableBody;
    if (!tbody) return;

    tbody.addEventListener('change', (event) => {
      const target = event.target;
      const field = target?.dataset?.collectorField;
      if (!field) return;
      const index = Number(target.dataset.index);
      if (!Number.isInteger(index) || index < 0 || index >= state.collectorPreviewItems.length) return;
      const item = state.collectorPreviewItems[index];
      ...
      if (field === 'quantity') {
        const quantity = parseCollectorQuantity(target.value);
        if (!quantity) {
          target.value = String(item.quantity || 1);
          showToast('Informe uma quantidade valida para importacao.', 'warning', 3000);
          return;
        }
  ...
  async function parseCollectorFile(fileContent) {
    ...
    if (!lines.length) {
      throw new Error('O arquivo informado esta vazio.');
    }
    ...
    lines.forEach((line, idx) => {
      ...
      const code = normalizeIdentifier(parts[0]);
      const quantityRaw = parts.slice(1).join(',').trim();
      const quantity = parseCollectorQuantity(quantityRaw);
      if (!code || !quantity) {
        malformedLines.push(lineNumber);
        return;
      }
      ...
      const current = groupedEntries.get(mapKey);
      groupedEntries.set(mapKey, {
        code,
    ...
    const previewItems = await Promise.all(
      Array.from(groupedEntries.values()).map(async (entry) => {
        try {
          const found = await findProductByCodeOrBarcode(entry.code);
          if (!found?._id) {
            return {
              inputCode: entry.code,
          ...
          const details = await fetchProductDetails(found._id);
          return {
            ...details,
            inputCode: entry.code,
            quantity: entry.quantity,
          ...
          return {
            ...
            selected: false,
            error: true,
            errorMessage: error?.message || 'Erro ao carregar produto',
          };
        }
      })
    );
    ...
    if (malformedLines.length) {
      showToast(`Linhas ignoradas por formato invalido: ${malformedLines.join(', ')}.`, 'warning', 5000);
    }

    return previewItems;
  }

  async function handleCollectorFileChange(event) {
    const file = event?.target?.files?.[0];
    if (!file) return;
    if (state.isReadOnly) return;

    if (!/\.txt$/i.test(file.name)) {
      showToast('Selecione um arquivo TXT valido.', 'warning', 4000);
      event.target.value = '';
      return;
    }

    try {
      if (selectors.collectorImportButton) {
        selectors.collectorImportButton.disabled = true;
        selectors.collectorImportButton.classList.add('opacity-70', 'cursor-not-allowed');
      }
      if (selectors.collectorTableBody) {
        selectors.collectorTableBody.innerHTML = '<tr><td colspan="6" class="px-4 py-8 text-center text-[11px] text-primary"><i class="fas fa-spinner fa-spin mr-2"></i>Carregando itens do arquivo...</td></tr>';
      }

      const content = await file.text();
      const previewItems = await parseCollectorFile(content);
      state.collectorPreviewItems = previewItems;
      renderCollectorPreviewTable();

      const foundCount = previewItems.filter((item) => !item.error).length;
      const missingCount = previewItems.filter((item) => item.error).length;
      showToast(`Arquivo lido: ${foundCount} item(ns) encontrados${missingCount ? `, ${missingCount} nao encontrado(s)` : ''}.`, 'info', 4500);
    } catch (error) {
      console.error('Erro ao processar arquivo do coletor:', error);
      state.collectorPreviewItems = [];
      renderCollectorPreviewTable();
      showToast(error?.message || 'Nao foi possivel processar o arquivo do coletor.', 'error', 5000);
    } finally {
      if (selectors.collectorImportButton) {
        selectors.collectorImportButton.disabled = false;
        selectors.collectorImportButton.classList.remove('opacity-70', 'cursor-not-allowed');
      }
    }
  }

  function importCollectorPreviewItems() {
    if (state.isReadOnly) return;
    if (!Array.isArray(state.collectorPreviewItems) || !state.collectorPreviewItems.length) {
      showToast('Carregue um arquivo antes de importar.', 'warning', 4000);
      return;
    }

    const itemsToImport = state.collectorPreviewItems.filter((item) => item.selected && !item.error);
    if (!itemsToImport.length) {
      showToast('Selecione ao menos um item valido para importar.', 'warning', 4000);
      return;
    }
    ...
    if (!originDepositId || !destinationDepositId) {
      showToast('Defina os depositos de origem e destino antes de importar itens.', 'warning', 4000);
      return;
    }

    let importedCount = 0;
    itemsToImport.forEach((selected) => {
      const existing = state.items.find((item) => item.productId === selected.productId);
      if (existing) {
        existing.quantity = Number(existing.quantity || 0) + Number(selected.quantity || 0);
        if ((normalizeFiniteNumber(existing.unitCost) === null || normalizeFiniteNumber(existing.unitCost) === 0) && normalizeFiniteNumber(selected.unitCost) !== null) {
          existing.unitCost = normalizeFiniteNumber(selected.unitCost);
        }
        if ((normalizeFiniteNumber(existing.unitSale) === null || normalizeFiniteNumber(existing.unitSale) === 0) && normalizeFiniteNumber(selected.unitSale) !== null) {
          existing.unitSale = normalizeFiniteNumber(selected.unitSale);
        }
      } else {
        state.items.push({
    ...
    resetCollectorPreview();
    showToast(`${importedCount} item(ns) importado(s) do coletor.`, 'success', 4000);
  }

  function attachCollectorImportEvents() {
    selectors.importCollectorButton?.addEventListener('click', () => {
      if (state.isReadOnly) return;
      resetCollectorPreview();
      setCollectorModalOpen(true);
    });

    selectors.collectorCancelButton?.addEventListener('click', () => {
      setCollectorModalOpen(false);
      resetCollectorPreview();
    });

    selectors.collectorModal?.addEventListener('click', (event) => {
      const actionElement = event.target?.closest?.('[data-action]');
      const action = actionElement?.dataset?.action;
      if (action === 'close-collector-modal') {
        setCollectorModalOpen(false);
        resetCollectorPreview();
      }
    });

    selectors.collectorFileInput?.addEventListener('change', handleCollectorFileChange);
    selectors.collectorImportButton?.addEventListener('click', importCollectorPreviewItems);

    document.addEventListener('keydown', (event) => {
      if (event.key === 'Escape' && selectors.collectorModal && !selectors.collectorModal.classList.contains('hidden')) {
        setCollectorModalOpen(false);
        resetCollectorPreview();
      }
    });
