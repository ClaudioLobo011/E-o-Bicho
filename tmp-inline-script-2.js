    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      const mainContent = document.getElementById('nfe-main-content');
      const fixedActionsBar = document.getElementById('nfe-fixed-actions-bar');
      ...
      const attachSearchTriggerToFilterInput = (input) => {
        ...
        trigger.type = 'button';
        trigger.className =
          'absolute left-2 top-1/2 -translate-y-1/2 text-[10px] text-gray-400 hover:text-primary focus:outline-none focus:ring-1 focus:ring-primary/30';
        trigger.setAttribute('aria-label', 'Focar filtro');
      ...
      const enhanceTableFilterInputsWithSearchTrigger = () => {
        const selectors = ['[data-main-products-filter]', '[data-code-lookup-filter]', '[data-price-filter]'];
        selectors.forEach((selector) => {
          document.querySelectorAll(selector).forEach((input) => attachSearchTriggerToFilterInput(input));
        });
      };

      const syncFixedActionsSpacing = () => {
        if (!mainContent || !fixedActionsBar) return;
        const barHeight = fixedActionsBar.offsetHeight || 0;
        const extraGap = 24;
        mainContent.style.paddingBottom = `${barHeight + extraGap}px`;
      };
      ...
      enhanceTableFilterInputsWithSearchTrigger();
      syncFixedActionsSpacing();
      window.addEventListener('resize', syncFixedActionsSpacing);
      window.setTimeout(syncFixedActionsSpacing, 0);
      ...
      if (importModal) {
        ...
        const priceRoundingButtons = priceModal ? priceModal.querySelectorAll('[data-price-round]') : [];
        const priceRoundingCustomWrapper = priceModal?.querySelector('[data-price-round-custom-wrapper]') || null;
        const priceRoundingCustomInput = priceModal?.querySelector('[data-price-round-custom-input]') || null;
        ...
        const priceExportExcelButton = priceModal?.querySelector('[data-price-export-excel]') || null;
        ...
        const companiesEndpoint = `${apiBaseUrl}/stores/allowed`;
        ...
        const isCompanyAllowed = (companyId) => {
          ...
          return companyRegistry.index.has(String(companyId));
        };
        ...
        const formatDecimalSuffixDisplay = (value) =>
        ...
        const ensureProductSupplierLink = (product, item, nfeData, options = {}) => {
          ...
          if (!forcePersist && productSupplierLinkCache.has(cacheKey)) {
            return null;
          }
        ...
        const PRODUCT_BARCODE_PRIMARY_FIELDS = [
          'codigoBarras',
          'codigoDeBarras',
          'barras',
          'ean',
        ...
        const PRODUCT_BARCODE_TRIBUTARY_FIELDS = [
          'eanTrib',
          'cEANTrib',
          'gtinTrib',
          'codigoBarrasTributado',
          'barcodeTributado',
          'codbarrasTributado',
        ];

        const getProductBarcodesFromRecord = (product) => {
          const results = [];
          const appendBarcode = (value) => {
            const normalized = normalizeBarcodeForComparison(value);
            if (normalized && !results.includes(normalized)) {
              results.push(normalized);
            }
          };
          const collectFromSource = (source, fields) => {
            if (!source || typeof source !== 'object') return;
            fields.forEach((field) => appendBarcode(source[field]));
          };
          ...
          collectFromSource(product, PRODUCT_BARCODE_PRIMARY_FIELDS);
          collectFromSource(product, PRODUCT_BARCODE_TRIBUTARY_FIELDS);

          if (Array.isArray(product.variacoes)) {
            product.variacoes.forEach((variation) => {
              collectFromSource(variation, PRODUCT_BARCODE_PRIMARY_FIELDS);
              collectFromSource(variation, PRODUCT_BARCODE_TRIBUTARY_FIELDS);
            });
          }
        ...
        const getProductBarcodeFromRecord = (product) => {
          const barcodes = getProductBarcodesFromRecord(product);
          return barcodes.length ? barcodes[0] : '';
        };

        const productMatchesBarcode = (product, barcodeDigits) => {
          const normalized = normalizeBarcodeForComparison(barcodeDigits);
          if (!normalized) return false;
          const productBarcode = normalizeBarcodeForComparison(product?.codbarras);
          return Boolean(productBarcode) && productBarcode === normalized;
        };
        ...
        const setApproveButtonState = (isProcessing) => {
          ...
          if (registerEntryText) {
            if (isProcessing) {
              ...
              registerEntryText.textContent = defaultRegisterText || 'Gravar';
            }
          }
        };
        ...
        const shouldSkipFormControlLock = (element) => {
          ...
          if (element.closest('#price-adjust-modal')) return true;
          return false;
        };
        ...
        const shouldSkipButtonLock = (button) => {
          ...
          if (button.hasAttribute('data-clear-import')) return true;
          if (button.closest('#price-adjust-modal')) return true;
          return false;
        };
        ...
        const sanitizeDecimalSuffix = (value) => {
          const numeric = toNumericValue(value);
          if (!Number.isFinite(numeric)) return null;
          if (numeric < 0) return 0;
          let normalized = numeric;
          if (normalized >= 1) {
            const fractional = normalized % 1;
            if (fractional > 0) {
              normalized = fractional;
            } else {
              normalized = (normalized % 100) / 100;
            }
          }
          normalized = Math.min(Math.max(normalized, 0), 0.99);
        ...
        const resolveProductNumericField = (product, fields) => {
          if (!product || typeof product !== 'object' || !Array.isArray(fields)) {
            return null;
          }
          for (const field of fields) {
            ...
            const numericValue = toNumericValue(directValue);
        ...
        const resolveProductCostValue = (product) =>
          resolveProductNumericField(product, [
            'custo',
            'precoCompra',
            'precoCusto',
            'valorCompra',
        ...
        const resolveProductSaleValue = (product) =>
          resolveProductNumericField(product, [
            'venda',
            'precoVenda',
            'valorVenda',
        ...
        const normalizeProductPricing = (product) => {
          if (!product || typeof product !== 'object') {
            return { cost: null, sale: null };
          }
          const cost = resolveProductCostValue(product);
          const sale = resolveProductSaleValue(product);
        ...
        const buildPriceAdjustmentItems = () => {
          ...
          sourceItems.forEach((item, index) => {
            ...
            const costPrevious = sanitizeCurrencyValue(resolveProductCostValue(item.matchedProduct));
            const salePrevious = sanitizeCurrencyValue(resolveProductSaleValue(item.matchedProduct));
        ...
        const recomputeEntryPricing = (entry, { source } = {}) => {
          ...
          const previousMarkup = Number.isFinite(entry.markupPrevious)
            ? entry.markupPrevious
            : computeMarkupPercentage(entry.costPrevious, entry.salePrevious);

          const resolveMarkup = (value) => {
            if (Number.isFinite(value)) return value;
            if (Number.isFinite(previousMarkup)) return previousMarkup;
            const derived = computeMarkupPercentage(costValue, entry.saleNew);
            return Number.isFinite(derived) ? derived : 0;
          };

          if (priceAdjustmentState.maintainMarkup) {
            const markupToApply = Number.isFinite(previousMarkup)
              ? previousMarkup
              : resolveMarkup(entry.markupCurrent);
            entry.markupCurrent = Number.isFinite(markupToApply) ? markupToApply : 0;
            entry.saleNew = computeSaleFromMarkup(costValue, entry.markupCurrent);
          } else if (priceAdjustmentState.usarMarkup) {
            if (source === 'sale-update') {
              entry.markupCurrent = resolveMarkup(computeMarkupPercentage(costValue, entry.saleNew));
            } else if (source === 'markup-update') {
              entry.markupCurrent = resolveMarkup(entry.markupCurrent);
              entry.saleNew = computeSaleFromMarkup(costValue, entry.markupCurrent);
            } else if (source === 'cost-update') {
              entry.markupCurrent = resolveMarkup(entry.markupCurrent);
              entry.saleNew = computeSaleFromMarkup(costValue, entry.markupCurrent);
            } else if (source === 'toggle-use-markup' || source === 'maintain-toggle') {
              const markupToApply = Number.isFinite(previousMarkup)
                ? previousMarkup
                : resolveMarkup(entry.markupCurrent);
              entry.markupCurrent = Number.isFinite(markupToApply) ? markupToApply : 0;
              ...
              entry.markupCurrent = resolveMarkup(entry.markupCurrent);
              if (!Number.isFinite(entry.saleNew)) {
                entry.saleNew = computeSaleFromMarkup(costValue, entry.markupCurrent);
              }
            }
          } else {
            if (source === 'markup-update') {
              entry.markupCurrent = resolveMarkup(entry.markupCurrent);
              entry.saleNew = computeSaleFromMarkup(costValue, entry.markupCurrent);
            } else if (source === 'sale-update') {
              entry.markupCurrent = resolveMarkup(computeMarkupPercentage(costValue, entry.saleNew));
            } else {
              const shouldRestoreSalePrevious =
                source === 'toggle-use-markup' && Number.isFinite(entry.salePrevious);

              if (shouldRestoreSalePrevious) {
                entry.saleNew = entry.salePrevious;
              } else if (!Number.isFinite(entry.saleNew) || source === 'cost-update') {
                const fallbackSale = Number.isFinite(entry.salePrevious)
                  ? entry.salePrevious
                  : computeSaleFromMarkup(costValue, resolveMarkup(entry.markupCurrent));
                entry.saleNew = Number.isFinite(fallbackSale) ? fallbackSale : 0;
              }
          ...
          if (!Number.isFinite(entry.saleNew)) {
            entry.saleNew = Number.isFinite(entry.salePrevious)
              ? entry.salePrevious
              : computeSaleFromMarkup(costValue, resolveMarkup(entry.markupCurrent));
          }
        ...
        const filterPriceItems = (items) => {
          ...
          return items.filter((entry) =>
            activeFilters.every(([key, value]) => {
              ...
              return matchesGlobalLikeFilter(columnValue, value);
            })
          );
        };
        ...
        const createPriceTableRow = (entry) => {
          ...
          costInput.addEventListener('blur', () => {
            ...
            entry.costNew = value;
            recomputeEntryPricing(entry, { source: 'cost-update' });
            costInput.value = entry.displays.costNew;
          ...
          const markupReadOnly = priceAdjustmentState.maintainMarkup;
          ...
          markupInput.addEventListener('blur', () => {
            ...
            recomputeEntryPricing(entry, { source: 'markup-update' });
            renderPriceAdjustmentTable();
          });
          ...
          const saleReadOnly = priceAdjustmentState.maintainMarkup;
        ...
        const downloadPriceAdjustmentTableAsExcel = () => {
          const items = Array.isArray(priceAdjustmentState.items) ? priceAdjustmentState.items : [];
          const filteredItems = filterPriceItems(items);
          if (!filteredItems.length) {
            notify('Não há itens na tabela para exportar.', 'warning');
            return;
          }
          ...
          filteredItems.forEach((entry) => {
            rows.push([
              entry.code || '—',
              entry.description || '—',
              entry.displays?.costPrevious || '—',
              entry.displays?.costNew || '',
              entry.displays?.markup || '',
              entry.displays?.saleNew || '',
              entry.displays?.salePrevious || '—',
              entry.displays?.entryStock || '—',
              entry.displays?.currentStock || '—',
              entry.displays?.previousStock || '—',
              entry.nfeItem?.taxRuleName || '—',
            ]);
          });
        ...
        const setUseMarkupState = (active, { silent } = {}) => {
          ...
          if (!silent) {
            recomputeAllEntries({ source: 'toggle-use-markup' });
            renderPriceAdjustmentTable();
          }
        };
        ...
        const setMaintainMarkupState = (active) => {
          ...
          recomputeAllEntries({ source: 'maintain-toggle' });
          renderPriceAdjustmentTable();
        };
        ...
        const roundPriceValue = (value, mode, { customSuffix } = {}) => {
          const numericValue = sanitizeCurrencyValue(value);
          if (!Number.isFinite(numericValue)) return null;
          if (mode === 'up') {
            const rounded = Math.ceil(numericValue / 0.5) * 0.5;
            return Math.max(Math.round(rounded * 100) / 100, 0);
          }
          if (mode === 'down') {
            const rounded = Math.floor(numericValue / 0.5) * 0.5;
            return Math.max(Math.round(rounded * 100) / 100, 0);
          }
          if (mode === 'custom') {
            const suffix = Number.isFinite(customSuffix) ? customSuffix : null;
            if (suffix === null) return null;
            const integerPart = Math.trunc(numericValue);
            let candidate = integerPart + suffix;
            if (candidate + 0.0001 < numericValue) {
              candidate = integerPart + 1 + suffix;
            }
            return Math.max(Math.round(candidate * 100) / 100, 0);
          }
          return null;
        };

        const setRoundingMode = (mode) => {
          const allowedModes = ['up', 'down', 'custom'];
          const normalized = allowedModes.includes(mode) ? mode : 'up';
          priceAdjustmentState.roundingMode = normalized;
          priceRoundingButtons.forEach((button) => {
            if (!(button instanceof HTMLButtonElement)) return;
            const isActive = button.dataset.priceRound === normalized;
            button.setAttribute('aria-pressed', isActive ? 'true' : 'false');
            button.classList.toggle('bg-primary/10', isActive);
            button.classList.toggle('border-primary/60', isActive);
            button.classList.toggle('text-primary', isActive);
          });
          if (priceRoundingCustomWrapper) {
            priceRoundingCustomWrapper.classList.toggle('opacity-70', normalized !== 'custom');
          }
          if (priceRoundingCustomInput) {
            priceRoundingCustomInput.classList.toggle('bg-gray-100', normalized !== 'custom');
          }
        };

        const applyRoundingToSelection = (mode) => {
          const allowedModes = ['up', 'down', 'custom'];
          const normalized = allowedModes.includes(mode)
            ? mode
            : priceAdjustmentState.roundingMode || 'up';
          if (normalized === 'custom') {
            const suffix = Number.isFinite(priceAdjustmentState.roundingCustomSuffix)
              ? priceAdjustmentState.roundingCustomSuffix
              : null;
            if (suffix === null) {
              notify('Informe uma decimal válida para aplicar o arredondamento personalizado.', 'warning');
          ...
          if (priceAdjustmentState.maintainMarkup) {
            notify('Desative a opção Manter markup para arredondar os preços de venda.', 'warning');
            return;
          }
          const selectedItems = priceAdjustmentState.items.filter((entry) => entry.selected);
          if (!selectedItems.length) {
            notify('Selecione ao menos um produto para arredondar os preços.', 'warning');
            return;
          }
          selectedItems.forEach((entry) => {
            const suffix = priceAdjustmentState.roundingCustomSuffix;
            const rounded = roundPriceValue(entry.saleNew, normalized, { customSuffix: suffix });
            if (!Number.isFinite(rounded)) return;
            entry.saleNew = rounded;
            recomputeEntryPricing(entry, { source: 'sale-update' });
          });
          renderPriceAdjustmentTable();
        };
        ...
        const openPriceAdjustmentModal = () => {
          ...
          priceAdjustmentState.roundingMode = priceAdjustmentState.roundingMode || 'up';
          setRoundingMode(priceAdjustmentState.roundingMode);
          if (priceRoundingCustomInput) {
            const suffix = Number.isFinite(priceAdjustmentState.roundingCustomSuffix)
              ? priceAdjustmentState.roundingCustomSuffix
              : null;
            priceRoundingCustomInput.value = suffix !== null ? formatDecimalSuffixDisplay(suffix) : '';
          }
          if (priceMaintainMarkupToggle) {
        ...
        const handlePriceUpdate = async () => {
          ...
          try {
            for (const entry of selectedItems) {
              ...
              const headers = buildAuthHeaders();
              headers.set('Content-Type', 'application/json');
              headers.set('X-Price-History-Screen', 'Entrada de NF-e');
        ...
        if (priceExportExcelButton) {
          priceExportExcelButton.addEventListener('click', (event) => {
            event.preventDefault();
            downloadPriceAdjustmentTableAsExcel();
          });
        }
        ...
        if (priceRoundingCustomInput) {
          priceRoundingCustomInput.addEventListener('blur', () => {
            const value = sanitizeDecimalSuffix(priceRoundingCustomInput.value);
            priceAdjustmentState.roundingCustomSuffix = Number.isFinite(value) ? value : null;
            priceRoundingCustomInput.value = Number.isFinite(value) ? formatDecimalSuffixDisplay(value) : '';
          });
        }
        ...
        priceRoundingButtons.forEach((button) => {
          ...
          button.addEventListener('click', (event) => {
            ...
            if (mode === 'advanced') {
              notify('O modo avançado de arredondamento ainda está em desenvolvimento.', 'info');
              return;
            }
            if (mode === 'custom') {
              const suffix = sanitizeDecimalSuffix(
                priceRoundingCustomInput ? priceRoundingCustomInput.value : priceAdjustmentState.roundingCustomSuffix
              );
              if (!Number.isFinite(suffix)) {
                notify('Informe uma decimal válida (ex.: 0,80) para arredondamento personalizado.', 'warning');
                if (priceRoundingCustomInput instanceof HTMLInputElement) {
                  priceRoundingCustomInput.focus();
                }
                return;
              }
              priceAdjustmentState.roundingCustomSuffix = suffix;
              if (priceRoundingCustomInput) {
                priceRoundingCustomInput.value = formatDecimalSuffixDisplay(suffix);
              }
            }
            setRoundingMode(mode);
            applyRoundingToSelection(mode);
          });
        });
        ...
        const resolveProductLinkDefaultTerm = (item) => {
          if (!item) return '';

          const tributaryBarcode = normalizeGtin(item?.eanTrib);
          if (tributaryBarcode) {
            return tributaryBarcode;
          }

          const fallbackBarcode = normalizeGtin(item?.ean);
          if (fallbackBarcode) {
            return fallbackBarcode;
          }

          const supplierCode = normalizeSupplierProductCode(item?.supplierCode);
          if (supplierCode) {
            return supplierCode;
          }
        ...
        const openProductLinkModal = (item) => {
          ...
          if (productLinkSearchInput) {
            const defaultTerm = resolveProductLinkDefaultTerm(item);
        ...
        const handleProductLinkSearch = async (rawTerm) => {
          ...
          try {
            ...
            const products = Array.isArray(payload?.products)
              ...
              ? payload
              : [];
            const filteredProducts = products.filter((product) => !product?.inativo);
            if (!filteredProducts.length) {
              if (productLinkResults) {
                productLinkResults.innerHTML = '';
              }
              showProductLinkEmpty('Nenhum produto ativo encontrado para os termos informados.');
              return;
            }
            renderProductLinkResults(filteredProducts);
          } catch (error) {
        ...
        const buildConversionCell = (item) => {
          ...
          const createNumericEditor = (labelText, { initialValue, allowZero = false, step = 1, quickActions = [], onCommit }) => {
            ...
            const createStepButton = (symbol, direction) => {
              ...
              button.title =
                direction > 0
                  ? 'Aumentar valor (Shift=x10, Alt=/10, Ctrl=/100)'
                  : 'Reduzir valor (Shift=x10, Alt=/10, Ctrl=/100)';
              button.addEventListener('click', (event) => {
                event.preventDefault();
          ...
          if (Number.isFinite(supplierFactor)) {
            ...
            const supplierTitle = supplierCalcType === 'divide'
              ...
              : `Aplicar cálculo do fornecedor (x ${supplierValueLabel})`;
            quickActions.push({
              label: 'Fornecedor',
        ...
        const matchesGlobalLikeFilter = (columnValue, rawFilter) => {
          const normalizedFilter = normalizeFilterText(rawFilter);
          if (!normalizedFilter) return true;
          const normalizedColumn = normalizeFilterText(columnValue);

          const globalRegexBuilder =
            (typeof window !== 'undefined' && typeof window.buildAdminTableFilterRegex === 'function'
              ? window.buildAdminTableFilterRegex
              : typeof buildAdminTableFilterRegex === 'function'
              ? buildAdminTableFilterRegex
              : null);

          if (globalRegexBuilder) {
            const regex = globalRegexBuilder(rawFilter);
            if (regex) {
              return regex.test(normalizedColumn);
            }
          }
              ...
              activeFilters.every(([key, raw]) => {
                ...
                const columnValue =
                  (typeof column.getFilterValue === 'function'
                    ...
                    : '') ?? '';
                return matchesGlobalLikeFilter(columnValue, filterValue);
              })
            );
          } else {
        ...
        const initializeProductTableHeader = () => {
          ...
          PRODUCT_TABLE_COLUMNS.forEach((column) => {
            ...
            attachSearchTriggerToFilterInput(filterInput);
        ...
        const findProductByBarcode = async (barcodeDigits) => {
          ...
          const promise = (async () => {
            const response = await fetch(
              `${apiBaseUrl}/products?search=${encodeURIComponent(normalized)}&limit=8&includeHidden=true`,
              { headers: buildAuthHeaders() }
            );
            const payload = await response.json().catch(() => ({}));
            if (!response.ok) {
            ...
            const products = Array.isArray(payload?.products)
              ...
              ? payload
              : [];
            const match = products.find((product) => productMatchesBarcode(product, normalized)) || null;
        ...
        const validateImportedProducts = async (nfeData) => {
          ...
          for (const item of items) {
            ...
            if (matched && matchedProduct) {
              normalizeProductPricing(matchedProduct);
        ...
        const buildMainProductRowData = (item, index) => {
          ...
          const ncmDisplay = item?.ncm || '—';
          const cestDisplay = item?.cest || '—';
          const cfopDisplay = item?.cfop || '—';
          const cstDisplay = item?.cst || '—';
          ...
          const pisRateDisplay = formatPercentValue(item?.pisRate);
          const cofinsRateDisplay = formatPercentValue(item?.cofinsRate);
          const quantityValue = toNumeric(item?.quantity);
          ...
          return {
            ...
            displays: {
              ...
              cfop: cfopDisplay,
              cst: cstDisplay,
              icmsRate: icmsRateDisplay,
              icmsStRate: icmsStRateDisplay,
              ipiRate: ipiRateDisplay,
              pisRate: pisRateDisplay,
              cofinsRate: cofinsRateDisplay,
            ...
            filterTexts: {
              ...
              unitNfe: unitDisplay !== '-' ? unitDisplay : '',
              ncm: ncmDisplay !== '-' ? ncmDisplay : '',
              cest: cestDisplay !== '-' ? cestDisplay : '',
              cfop: cfopDisplay !== '-' ? cfopDisplay : '',
              cst: cstDisplay !== '-' ? cstDisplay : '',
              icmsRate: buildNumericFilterValue(icmsRateDisplay, item?.icmsRate),
              icmsStRate: buildNumericFilterValue(icmsStRateDisplay, item?.icmsStRate),
              ipiRate: buildNumericFilterValue(ipiRateDisplay, item?.ipiRate),
              pisRate: buildNumericFilterValue(pisRateDisplay, item?.pisRate),
              cofinsRate: buildNumericFilterValue(cofinsRateDisplay, item?.cofinsRate),
              barcode: effectiveBarcodeDisplay || '',
              supplierDescription: supplierDescriptionDisplay,
            ...
            sortValues: {
              ...
              unitNfe: unitSortValue,
              ncm: ncmDisplay,
              cest: cestDisplay,
              cfop: cfopDisplay,
              cst: cstDisplay,
              icmsRate: toNumeric(item?.icmsRate),
              icmsStRate: toNumeric(item?.icmsStRate),
              ipiRate: toNumeric(item?.ipiRate),
              pisRate: toNumeric(item?.pisRate),
              cofinsRate: toNumeric(item?.cofinsRate),
              barcode: effectiveBarcodeDisplay || '',
              supplierDescription: supplierDescriptionDisplay,
        ...
        const updateMainProductsTableDisplay = () => {
          ...
          itemsToRender.forEach((entry) => {
            ...
            ncmCell.className = 'px-4 py-3 text-left text-gray-600';
            ncmCell.textContent = displays.ncm;

            const cestCell = document.createElement('td');
            cestCell.className = 'px-4 py-3 text-left text-gray-600';
            cestCell.textContent = displays.cest;

            const cfopCell = document.createElement('td');
            cfopCell.className = 'px-4 py-3 text-left text-gray-600';
            cfopCell.textContent = displays.cfop;

            const cstCell = document.createElement('td');
            cstCell.className = 'px-4 py-3 text-left text-gray-600';
            cstCell.textContent = displays.cst;

            const icmsRateCell = document.createElement('td');
            icmsRateCell.className = 'px-4 py-3 text-right text-gray-600';
            icmsRateCell.textContent = displays.icmsRate;

            const icmsStRateCell = document.createElement('td');
            icmsStRateCell.className = 'px-4 py-3 text-right text-gray-600';
            icmsStRateCell.textContent = displays.icmsStRate;

            const ipiRateCell = document.createElement('td');
            ipiRateCell.className = 'px-4 py-3 text-right text-gray-600';
            ipiRateCell.textContent = displays.ipiRate;

            const pisRateCell = document.createElement('td');
            pisRateCell.className = 'px-4 py-3 text-right text-gray-600';
            pisRateCell.textContent = displays.pisRate;

            const cofinsRateCell = document.createElement('td');
            cofinsRateCell.className = 'px-4 py-3 text-right text-gray-600';
            cofinsRateCell.textContent = displays.cofinsRate;

            const gtinCell = document.createElement('td');
            gtinCell.className = 'px-4 py-3 text-left text-gray-600';
            gtinCell.textContent = displays.barcode || 'SEM GTIN';

            const supplierDescriptionCell = document.createElement('td');
            supplierDescriptionCell.className = 'px-4 py-3 text-left text-gray-600';
            supplierDescriptionCell.textContent = displays.supplierDescription || displays.description || '-';
        ...
        const applyCodeLookupFiltersToRows = (entries) => {
          ...
          return entries.filter((entry) =>
            filters.every(([key, raw]) => {
              const columnValue = entry.filterValues?.[key];
              return matchesGlobalLikeFilter(columnValue, raw);
            })
          );
        };
        ...
        const parseNfeXml = (xmlText) => {
          ...
          const items = detElements.map((detElement, index) => {
            ...
            const barcodeCandidates = Array.from(new Set([normalizeGtin(eanTrib), normalizeGtin(ean)].filter(Boolean)));
        ...
        if (companySelect) {
          companySelect.addEventListener('change', () => {
            ...
            if (selectedCompany && !isCompanyAllowed(selectedCompany)) {
              ...
              notify('Empresa nao autorizada para esta operacao.', 'warning');
              return;
            }
            pendingCompanyId = '';
            if (lastNfeData) {
